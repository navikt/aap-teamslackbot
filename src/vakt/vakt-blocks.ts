export function vaktBlocks(dagensTekniskeVakt: string[], dagensTestVakt: string) {
    const tekniskVaktTekst = `:guardsman: *Dagens tekniske vakt er ${dagensTekniskeVakt.map(vakt => `<@${vakt}>`).join(' ')}*\nGruppen <@aap-teknisk-vakt> oppdateres med dagens vakt og kan brukes i stedet for navnet til vakten`;    const logLink = "<https://logs.adeo.no/app/discover#/view/71a530ed-9b1d-4c5f-bc75-9efcc262323a|Elastic - feil i prod>";
