export function isTodayAHoliday() {
    ...
    const påskeStart = new Date(today.getFullYear(), 4, 17);
    const påskeSlutt = new Date(today.getFullYear(), 4, 21);
    ...
    return ((today >= julStart && today <= julSlutt) || (today >= nyttaarStart && today <= nyttaarSlutt)|| (today >= påskeStart && today <= påskeSlutt));
}